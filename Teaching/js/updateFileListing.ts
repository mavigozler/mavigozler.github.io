...
declare function loadPDF(url: string): void;

export type ServerConfig = {
	extension: string;
	path: string;
	staticRoot: string;
	filesFilter: (s: string) => boolean;
...
export const SERVER_CONFIG: ServerConfig[] = [
	{
		...
		filesFilter: (s: string) => s.endsWith(".pdf"),
	...
	},
	{
		...
		filesFilter: (s: string) => s.endsWith('.html'),
...
const LISTS_SELECT_LINES_MAX = 4,
	LIST_SELECT_LINES_MAX = 5;
let selectedConfig: ServerConfig;

if (typeof document != "undefined")
document.addEventListener("DOMContentLoaded", () => {
	const filesListsDiv: HTMLElement | null = document.getElementById("files-lists");
	if (filesListsDiv == null)
		appError("Missing an HTML 'div' element to put file listings");
	else {
		let listsSelectLinesMax = LISTS_SELECT_LINES_MAX,
			configIndex = 0,
			fileIndex: number;
		...
		listsSelectElem.addEventListener("change", (evt) => {
			const selectElem: HTMLSelectElement = evt.target as HTMLSelectElement;
			...
			config.fileNamesList.forEach((fileName) => {
				...
				optionElem.appendChild(document.createTextNode(
					fileName.search(extension) > 0 ?
		...
		oneListSelectElem.addEventListener("change", (evt) => {
			const selectElem: HTMLSelectElement = evt.currentTarget as HTMLSelectElement;
			if (selectedConfig.extension == "pdf") {
				const pdfViewingDiv = document.getElementById("pdf-viewing") as HTMLDivElement;
				...
				const webDocIframe: HTMLIFrameElement = document.getElementById("webdoc-viewing") as HTMLIFrameElement;
...
function appError(message: string) {
	let appErrorDiv: HTMLDivElement | null = document.getElementById("app-error") as HTMLDivElement;
	if (appErrorDiv == null) {
	...
	appErrorDiv.replaceChild(document.createTextNode(message), appErrorDiv.firstChild as ChildNode);
}