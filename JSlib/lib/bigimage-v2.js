...
export { htmlDocTypeDecl, xhtmlMetaContentTypeAsXHTML, xhtmlDocTypeDecl, htmlMetaContentTypeAsHTML, setThumbedImages, thumbedImage, getImage, setDocWideBigImages };
*/
 ...
 * @return void
 */
function resizeImage(imgObj, factor) {
...
*/
function setThumbedImages(docBody, force, includePrompt) {
    var _a, _b;
        ...
        for (let j = 0; j < classNames.length; j++)
            if (classNames[j].toLowerCase().search("thumbimage") >= 0) {
                ...
                if (factor) {
                    ...
                    intermediate = (_a = iCSS.doCssMath(MathOperation.CSS_MULTIPLY, -2, imgParentStyle.padding)) !== null && _a !== void 0 ? _a : calculationError();
                    intermediate = (_b = iCSS.doCssMath(MathOperation.CSS_SUM, imgParentStyle.width, intermediate)) !== null && _b !== void 0 ? _b : calculationError();
...
/* thumbedImage() for IMG onload event to size the thumb form of an image
   ...
   it should be like */
function thumbedImage(imgObj, screenProportion) {
    thumbIfBig(imgObj, screenProportion);
}
function thumbIfBig(imgObj, screenProportion) {
    if (!imgObj)
...
function resizeImage2Screen(imgObj, scrn, resizeFactor) {
...
function resizeWin2Image(imgObj, win, resizeFactor) {
    if (typeof imgObj == "undefined" || imgObj == null)
 ...
 */
function setDocWideBigImages(document, styling) {
    ...
    for (let i = 0; i < docImages.length; i++) {
        clonedImage = setImagePrompt(docImages[i], styling);
...
function docWideBigImage(evt) {
    if (evt.target)
        return bigimage(evt.target, true);
    return bigimage(evt.currentTarget, true);
}
...
function bigimage(imgObj, origRez) {
...
let imgWin; // recycle windows when possible
function makeBigImage(imgURL, imageTitle) {
    ...
    imgWin = window.open("" /*window.location.href*/, "", "resizable=no,scrollbars=no;,height=" +
...
function origImage(imgURL, imageTitle) {
    let origwin;
