...
function contentBuilder(config, htmldoc) {
    ...
    for (const baseRate in config.baseRates) {
        ...
        spanElem?.replaceChild(document.createTextNode("$" + config.baseRates[baseRate].toString()), spanElem.firstChild);
    }
    ...
    for (let row = 1; row < rows.length; row++) {
        ...
        cols[1].appendChild(document.createTextNode((100 - config.DollarReduction[row - 1] / config.DollarReduction[0] * 100).toFixed(0) + "%"));
    }
    document.getElementById("example-base")?.appendChild(document.createTextNode("$" + config.DollarReduction[0]));
}
