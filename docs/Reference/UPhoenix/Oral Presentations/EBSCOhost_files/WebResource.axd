...
function WebForm_DoPostBackWithOptions(options) {
    ...
    if (validationResult) {
        if ((typeof(options.actionUrl) != "undefined") && (options.actionUrl != null) && (options.actionUrl.length > 0)) {
            theForm.action = formUrl2Muse(theForm, options.actionUrl);
        }
...
function WebForm_DoCallback(eventTarget, eventArgument, eventCallback, context, errorCallback, useAsync) {
    ...
    if (setRequestHeaderMethodExists) {
        ...
        xmlRequest.open("POST", url2Muse(xmlRequest, museaction(theForm, theForm.action)), true);
    ...
    if (!xmlRequestFrame) {
        ...
        try {
            if (callBackFrameUrl) {
                xmlRequestFrame.src = srcUrl2Muse(xmlRequestFrame, callBackFrameUrl);
            }
        }
        catch(e) {}
    ...
    var interval = window.setInterval(function() {
        ...
        if (xmlRequestFrame && xmlRequestFrame.document) {
            ...
            xmlRequestFrame.document.close();
            xmlRequestFrame.document.forms[0].action = formUrl2Muse(xmlRequestFrame.document.forms[0], museaction(theForm, theForm.action));
