...
function contentBuilder(htmldoc) {
    ...
    for (const baseRate of baseRates) {
        ...
        spanElem?.replaceChild(document.createTextNode("$" + baseRate.rate.toString()), spanElem.firstChild);
    }
    ...
    for (let row = 1; row < rows.length; row++) {
        ...
        cols[1].appendChild(document.createTextNode((100 - DollarReduction[row - 1] / DollarReduction[0] * 100).toFixed(0) + "%"));
    }
    document.getElementById("example-base")?.appendChild(document.createTextNode("$" + DollarReduction[0]));
}
